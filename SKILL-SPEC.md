...
运行时 Skill 统一放在 `eino-examples/quickstart/chatwitheino/skills/health/` 下；每个一级子目录就是一个可被 Eino skill middleware 扫描的 Skill，必须满足 `EINO_EXT_SKILLS_DIR/<skillName>/SKILL.md`。PM 草稿和迁移前素材可单独放在项目根目录的 `skills/` 或专题工作目录下，但不作为运行时扫描目录。

每个 PM 草稿可维护自己的 SKILL 目录，建议放在项目根目录的 `skills/` 下：
...
1. 把可运行的 SKILL 目录提交到 `eino-examples/quickstart/chatwitheino/skills/health/<skillName>/`
2. 确保每个一级子目录下都有 `SKILL.md`，因为当前框架只扫描 `EINO_EXT_SKILLS_DIR/*/SKILL.md`
3. `run.sh` 默认会把 `EINO_EXT_SKILLS_DIR` 指向内置健康管家 Skill 目录，无需额外修改框架代码
...
AI 回答后用于问诊/科普快测的信息采集卡。Skill 作者不直接输出 JSON，而是在回答末尾输出结构化标签，系统会解析为卡片，标签本身不展示给用户。

```text
<快捷回复> [单选] 红疹特征: 散在小红点 | 成片红斑 | 有水疱或脱皮 | 风团样隆起 </快捷回复>
<图片收集>拍一张患处近照，重点看皮损形态、边界和表面变化</图片收集>
```

**适用场景**：问诊场景快测、科普场景快测、需要用户快捷选择或上传图片补充信息。

- `[单选]` / `[多选]` — 控制选项选择方式
- `标签名` — 卡片主问题，例如“红疹特征”
- `选项` — 用 `|` 分隔，建议不超过 6 个
- `<图片收集>` — 可选，和 `<快捷回复>` 同轮组合展示
...
output_cards:
  ...
  - type: quick_test
    trigger: "需要用户快捷选择、补充问诊信息或上传图片时"
---
```

### 8.3 System Prompt 中的卡片输出指令

在 System Prompt 中告诉模型何时输出卡片。当前 demo 对 PM 友好的结构化标签做运行时解析，禁止把 JSON 卡片、XML/HTML 标签直接作为用户可见正文输出。

```markdown
### 卡片输出规则

当需要用户补充关键信息时，在文本回答末尾输出：

<快捷回复> [单选] 问题标题: 选项1 | 选项2 | 选项3 </快捷回复>

当需要用户上传图片时，在文本回答末尾追加：

<图片收集>拍摄说明</图片收集>

规则：
- 用户明确要问医生 → 可输出服务承接文案，由前端服务卡承接
- 需要问诊/科普快测 → 输出 quick_test 标签协议
- 标签必须放在正文最后，不要嵌入正文段落
```
